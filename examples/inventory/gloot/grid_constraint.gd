...
const Verify = preload("./verify.gd")
const GridConstraint = preload("./grid_constraint.gd")
const StacksConstraint = preload("./stacks_constraint.gd")
const ItemMap = preload("./item_map.gd")
...
@export var size: Vector2i = DEFAULT_SIZE :
    set(new_size):
        assert(inventory, "GlootInventory not set!")
        assert(new_size.x > 0, "GlootInventory width must be positive!")
        assert(new_size.y > 0, "GlootInventory height must be positive!")
        var old_size = size
        size = new_size
...
func set_item_position(item: GlootInventoryItem, new_position: Vector2i) -> bool:
...
static func is_item_rotated(item: GlootInventoryItem) -> bool:
    return item.get_property(KEY_ROTATED, false)


static func is_item_rotation_positive(item: GlootInventoryItem) -> bool:
    return item.get_property(KEY_POSITIVE_ROTATION, false)


# TODO: Consider making a static "unsafe" version of this
func set_item_size(item: GlootInventoryItem, new_size: Vector2i) -> bool:
...
static func set_item_rotation_direction(item: GlootInventoryItem, positive: bool) -> void:
    if positive:
...
func set_item_rect(item: GlootInventoryItem, new_rect: Rect2i) -> bool:
...
func _get_prototype_size(prototype_id: String) -> Vector2i:
    ...
    assert(inventory.item_protoset != null, "GlootInventory protoset is null!")
...
func move_item_to_free_spot(item: GlootInventoryItem) -> bool:
    if rect_free(get_item_rect(item), item):
        return true
...
func _move_item_to_unsafe(item: GlootInventoryItem, position: Vector2i) -> void:
...
func transfer_to(item: GlootInventoryItem, destination: GridConstraint, position: Vector2i) -> bool:
    assert(inventory != null, "GlootInventory not set!")
...
func _merge_to(item: GlootInventoryItem, destination: GridConstraint, position: Vector2i) -> bool:
...
func _get_mergable_item_at(item: GlootInventoryItem, position: Vector2i) -> GlootInventoryItem:
...
func _get_mergable_items_under(item: GlootInventoryItem, rect: Rect2i) -> Array[GlootInventoryItem]:
    var result: Array[GlootInventoryItem]
...
func find_free_place(item: GlootInventoryItem, exception: GlootInventoryItem = null) -> Dictionary:
...
func _compare_items(item1: GlootInventoryItem, item2: GlootInventoryItem) -> bool:
...
func sort() -> bool:
    ...
    var item_array: Array[GlootInventoryItem]
    for item in inventory.get_items():
...
func get_space_for(item: GlootInventoryItem) -> GlootItemCount:
    ...
    if item_size == Vector2i.ONE:
        return GlootItemCount.new(_item_map.free_fields)
    ...
    return GlootItemCount.new(occupied_rects.size())
