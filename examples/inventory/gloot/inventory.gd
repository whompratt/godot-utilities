...
const ConstraintManager = preload("./constraint_manager.gd")

@export var item_protoset: GlootItemProtoset:
...
var _items: Array[GlootInventoryItem] = []
...
func _enter_tree():
	for child in get_children():
		if not child is GlootInventoryItem:
			continue
		if has_item(child):
			continue
		_items.append(child)
...
func get_item_index(item: GlootInventoryItem) -> int:
...
func _connect_item_signals(item: GlootInventoryItem) -> void:
...
func _disconnect_item_signals(item:GlootInventoryItem) -> void:
	if item.protoset_changed.is_connected(_emit_item_modified):
...
func _emit_item_modified(item: GlootInventoryItem) -> void:
	_constraint_manager._on_item_modified(item)
	item_modified.emit(item)


func get_items() -> Array[GlootInventoryItem]:
	return _items


func has_item(item: GlootInventoryItem) -> bool:
	return item in _items


func add_item(item: GlootInventoryItem) -> bool:
...
func can_add_item(item: GlootInventoryItem) -> bool:
...
func create_and_add_item(prototype_id: String) -> GlootInventoryItem:
	var item: GlootInventoryItem = GlootInventoryItem.new()
...
func remove_item(item: GlootInventoryItem) -> bool:
...
func _can_remove_item(item: GlootInventoryItem) -> bool:
...
func get_item_by_id(prototype_id: String) -> GlootInventoryItem:
	for item in get_items():
...
func get_items_by_id(prototype_id: String) -> Array[GlootInventoryItem]:
...
func transfer(item: GlootInventoryItem, destination: GlootInventory) -> bool:
...
func deserialize(source: Dictionary) -> bool:
	...
	if source.has(KEY_ITEMS):
		...
		for item_dict in items:
			...
			assert(add_item(item), "Failed to add item '%s'. GlootInventory full?" % item.prototype_id)
