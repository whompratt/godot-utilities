@tool
class_name GlootItemSlot
extends "./item_slot_base.gd"
...
func _return_item_to_source_inventory() -> bool:
    var inventory: GlootInventory = (_wr_source_inventory.get_ref() as GlootInventory)
    if inventory != null:
...
func deserialize(source: Dictionary) -> bool:
    ...
    if source.has(KEY_ITEM):
        var item := GlootInventoryItem.new()
