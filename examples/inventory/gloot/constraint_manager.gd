...
var inventory: GlootInventory = null :
	set(new_inventory):
		...
		assert(inventory == null, "GlootInventory already set!")
...
func _enforce_constraints(item: GlootInventoryItem) -> bool:
...
func get_space_for(item: GlootInventoryItem) -> GlootItemCount:
	match get_configuration():
		...
		Configuration.WG:
			return GlootItemCount.min(_grid_constraint.get_space_for(item), _weight_constraint.get_space_for(item))
		Configuration.SG:
			return _sg_get_space_for(item)
		Configuration.WSG:
			return GlootItemCount.min(_sg_get_space_for(item), _ws_get_space_for(item))

	return GlootItemCount.inf()


func _ws_get_space_for(item: GlootInventoryItem) -> GlootItemCount:
	var stack_size := GlootItemCount.new(_stacks_constraint.get_item_stack_size(item))
	var result := _weight_constraint.get_space_for(item).div(stack_size)
	return result


func _sg_get_space_for(item: GlootInventoryItem) -> GlootItemCount:
	var grid_space := _grid_constraint.get_space_for(item)
	var max_stack_size := GlootItemCount.new(_stacks_constraint.get_item_max_stack_size(item))
	var stack_size := GlootItemCount.new(_stacks_constraint.get_item_stack_size(item))
	var free_stacks_space: GlootItemCount = _stacks_constraint.get_free_stack_space_for(item)
...
func _sg_has_space_for(item: GlootInventoryItem) -> bool:
