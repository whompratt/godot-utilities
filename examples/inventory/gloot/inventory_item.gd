...
@export var protoset: GlootItemProtoset :
    set(new_protoset):
...
var _inventory: GlootInventory
var _item_slot: GlootItemSlot
...
func _on_parented(parent: Node) -> void:
    if parent is GlootInventory:
        _on_added_to_inventory(parent as GlootInventory)
    else:
        _inventory = null

    if parent is GlootItemSlot:
        _link_to_slot(parent as GlootItemSlot)
    else:
        _unlink_from_slot()


func _on_added_to_inventory(inventory: GlootInventory) -> void:
...
func _on_removed_from_inventory(inventory: GlootInventory) -> void:
    if inventory:
        removed_from_inventory.emit(inventory)
        inventory._on_item_removed(self)


func _link_to_slot(item_slot: GlootItemSlot) -> void:
...
func get_inventory() -> GlootInventory:
    return _inventory
