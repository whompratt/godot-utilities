...
func _init(inventory: GlootInventory) -> void:
...
func _calculate_occupied_space() -> void:
	...
	if !Engine.is_editor_hint():
		assert(has_unlimited_capacity() || _occupied_space <= capacity, "GlootInventory overflow!")


static func _get_item_unit_weight(item: GlootInventoryItem) -> float:
	var weight = item.get_property(KEY_WEIGHT, 1.0)
	return weight


static func get_item_weight(item: GlootInventoryItem) -> float:
	if item == null:
		return -1.0
	return StacksConstraint.get_item_stack_size(item) * _get_item_unit_weight(item)


static func set_item_weight(item: GlootInventoryItem, weight: float) -> void:
...
func get_space_for(item: GlootInventoryItem) -> GlootItemCount:
	...
	return GlootItemCount.new(floor(get_free_space() / unit_weight))
