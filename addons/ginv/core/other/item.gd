...
@export var protoset: Protoset:
	set(new_protoset):
		if new_protoset == protoset:
			return
		if _inventory && _inventory.protoset != new_protoset:
			return
		...
		if protoset && protoset.prototypes && protoset.prototypes.keys().size() > 0:
			prototype_id = protoset.prototypes.keys()[0]
		else:
			prototype_id = ""
...
@export var prototype_id: String:
	set(new_prototype_id):
		if new_prototype_id == prototype_id:
			...
			return
		# TODO: Remove this crap check, replace with on_text_submitted?
