...
func get_free_stack_space_for(item: Item) -> int:
	...
	for mergeable_item in mergeable_items:
		item_count += ConstraintStack.get_free_space_for(mergeable_item)
	return item_count
...
func pack_item(item: Item):
	var free_space: int = get_free_stack_space_for(item)
...
static func get_free_space_for(item: Item) -> int:
...
static func get_prototype_stack_size(protoset: Protoset, prototype_id: String) -> int:
	var prototype = protoset.get_prototype(prototype_id)
	return protoset.get_property(prototype, KEY_STACK_SIZE, 1.0)

static func get_prototype_max_stack_size(protoset: Protoset, prototype_id: String) -> int:
	var prototype = protoset.get_prototype(prototype_id)
