...
func _get_prototype_size(prototype_id: String) -> Vector2i:
    ...
    var prototype = inventory.item_protoset.get_prototype(prototype_id)
    var width: int = inventory.item_protoset.get_property(prototype, KEY_WIDTH, 1)
    var height: int = inventory.item_protoset.get_property(prototype, KEY_HEIGHT, 1)
